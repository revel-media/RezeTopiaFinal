...
import io.krito.com.rezetopia.activities.PageSetting;
...
import io.krito.com.rezetopia.models.pojo.pages.Admins;
...
public class PageRecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public PageRecyclerAdapter(Context context, ArrayList<NewsFeedItem> items, long now, String userId, PageResponse pageResponse, String pageName, int pageId
    , ArrayList<Admin> pageAdmins) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        if (viewType == VIEW_HOME_HEADER) {
            View view = LayoutInflater.from(context).inflate(R.layout.page_header, parent, false);
    ...
    private class HeaderViewHolder extends RecyclerView.ViewHolder {
        ...
        CustomTextView members;
        CustomTextView admins;
        ExpandableRelativeLayout aboutExpandable;
        TextView aboutText;
        ...
        public HeaderViewHolder(View itemView) {
            ...
            cover = itemView.findViewById(R.id.coverView);
            pp = itemView.findViewById(R.id.ppView);
            name = itemView.findViewById(R.id.usernameView);
            about = itemView.findViewById(R.id.about);
            members = itemView.findViewById(R.id.friends);
            admins = itemView.findViewById(R.id.profilePhotos);
            aboutExpandable = itemView.findViewById(R.id.expandableLayoutAbout);
            ...
            likePage = itemView.findViewById(R.id.likePageBtn);
            sideMenu = itemView.findViewById(R.id.side_menu);

            sideMenu.setVisibility(View.GONE);

            for (Admin admin:pageResponse.getAdmins()) {
                if (admin.getId() == Integer.parseInt(userId)){
            ...
            sideMenu.setOnClickListener(v -> {
                ...
                menu.getMenuInflater().inflate(R.menu.setting_menu, menu.getMenu());
                menu.setOnMenuItemClickListener(item -> {
                    callback.onOpenSetting();
                    return false;
                });
            ...
            if (pageResponse.isLiked()){
                likePage.setText(R.string.unlike);
            } else {
                likePage.setText(R.string.like);
            }
            ...
            if (pageResponse != null){
                if (pageResponse.getCover() != null){
                    Picasso.with(context).load(pageResponse.getCover()).into(cover);
                }

                if (pageResponse.getPp() != null){
                    Picasso.with(context).load(pageResponse.getPp()).into(pp);
                }
                ...
                if (pageResponse.getAbout() != null){
                    if (pageResponse.getAbout().getAbout() != null && !pageResponse.getAbout().getAbout().isEmpty()){
                        aboutText.setText(pageResponse.getAbout().getAbout());
                    } else {
                        aboutText.setText("none");
                    }

                    if (pageResponse.getAbout().getAddress() != null && !pageResponse.getAbout().getAddress().isEmpty()){
                        addressText.setText(pageResponse.getAbout().getAddress());
                    } else {
                        addressText.setText("none");
                    }

                    if (pageResponse.getAbout().getPhone() != null && !pageResponse.getAbout().getPhone().isEmpty()){
                        phoneText.setText(pageResponse.getAbout().getPhone());
                    } else {
                        phoneText.setText("none");
                    }

                    if (pageResponse.getAbout().getWebsite() != null && !pageResponse.getAbout().getWebsite().isEmpty()){
                        websiteText.setText(pageResponse.getAbout().getWebsite());
                    } else {
                        websiteText.setText("none");
                    }
                }
            }

            about.setOnClickListener(v -> {
                if (aboutExpandable.isExpanded()){
                    aboutExpandable.collapse(500, new AccelerateDecelerateInterpolator());
                } else {
                    aboutExpandable.expand(500, new AccelerateDecelerateInterpolator());
                }
            });

            likePage.setOnClickListener(v -> {
                likePage.setEnabled(false);
                likePage.setAlpha(0.5f);
                String reverse = likePage.getText().toString().contentEquals(context.getResources().getString(R.string.like)) ?
                        context.getResources().getString(R.string.unlike) : context.getResources().getString(R.string.like);

                StringRequest request = new StringRequest(Request.Method.POST, "https://rezetopia.com/Apis/pages/like", new Response.Listener<String>() {
                    @Override
                    public void onResponse(String response) {
                        likePage.setAlpha(1.0f);
                        likePage.setEnabled(true);
                        Log.i("like_page_response", "onResponse: " + response);
                        try {
                            JSONObject jsonObject = new JSONObject(response);
                            if (jsonObject.getString("status").contentEquals("200")) {
                                likePage.setText(reverse);
                            } else {
                    ...
                    public void onErrorResponse(VolleyError error) {
                        Log.i("like_page_error", "onErrorResponse: " + error.getMessage());
                        likePage.setAlpha(1.0f);
                        likePage.setEnabled(true);
                    }
                }){
...
//            }
        ...
        private void performLike1(final NewsFeedItem item){
            String url = "https://rezetopia.com/Apis/likes/post";
        ...
        private void performLike(final NewsFeedItem item) {
            ...
            Log.i("page_post_like", "performLike: " + userId + "  " + item.getPostId());
...
//                                context.startActivity(intent);
    ...
    private void fetchAdmins(){
        VolleyCustomRequest request = new VolleyCustomRequest(Request.Method.GET, "https://rezetopia.com/Apis/pages/admins?page_id=" + pageId,
                Admins.class, new Response.Listener<Admins>() {
            @Override
            public void onResponse(Admins response) {
                Log.i("admins", "onResponse: " + response.isError());
                if (!response.isError()){
                    pageAdmins = new ArrayList<>(Arrays.asList(response.getAdmins()));
                }
            }
        }, new Response.ErrorListener() {
            @Override
            public void onErrorResponse(VolleyError error) {
                Log.i("admins", "onErrorResponse: " + error.getMessage());
    ...
    private void likePage(String reverse){
